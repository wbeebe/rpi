...
func main() {
	...
	// now reorder bits
	//
    fmt.Println("package ht16k33\n")
	fmt.Println("var VT52rom[][]byte = [][]byte{")

    // Load a character (8 bytes) worth of data.
    //
	for charnum, chardata := range vt52rom {

        // Load the data into an 8-byte work area.
        //
        ...
        for i := 0; i < 8; i++ {
		    var colByte byte;
            // Across every work byte in the work area,
            // shift out every high bit from every row in the work area,
            // and combine those bits into a column byte
            // that can be displayed on an Adafruit 8x8 display block.
            //
            for row, rowByte:= range reorder {
                colByte <<= 1
                colByte |= (rowByte & 0x80) >> 7
                reorder[row] <<= 1
            }

			fmt.Printf("0x%02x, ", colByte)
        }
